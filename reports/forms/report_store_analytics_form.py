...
class ReportStoreAnalyticsForm(forms.Form):
    ...
    def generate_report(self):
        ...
        select_cause = f"""
            ...
            round(SAFE_DIVIDE(sum(price), count(*), 0)) as average_price,
        """
        if selected_store:
            retailer_case_count = (
            ...
            select_cause += f"""
                ...
                round(SAFE_DIVIDE(count({retailer_case_count}), count(*), 0), 4) as precentage_clicks,
                sum({retailer_case_price}) as retailer_sum,
                sum(price) - sum({retailer_case_price}) as diff,
                round(SAFE_DIVIDE(sum({retailer_case_price}), sum(price), 0), 4) as percentage_sum,
