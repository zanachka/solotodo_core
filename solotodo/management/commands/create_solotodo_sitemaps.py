...
from solotodo.models import Product, Category, Subcategory
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        while True:
            products = Product.objects.filter_by_category(categories).select_related(
                "instance_model"
            ...
            for product in products:
                ...
                loc.text = "{}/products/{}-{}".format(domain, product.id, product.slug)
                lastmod = ET.SubElement(url, "lastmod")
                lastmod.text = product.last_updated.isoformat()

            et = ET.ElementTree(urlset)
            file = open(
                "sitemap_products_{}_{}.xml".format(page + 1, country_code), "wb"
            )
        ...
        for video_card_gpu in model.instancemodel_set.all():
            url = ET.SubElement(urlset, "url")
            loc = ET.SubElement(url, "loc")
            loc.text = "{}/video_card_gpus/{}".format(domain, video_card_gpu.id)
        ...
        for subcategory in Subcategory.objects.all():
            url = ET.SubElement(urlset, "url")
            loc = ET.SubElement(url, "loc")
            loc.text = "{}/{}".format(domain, subcategory.slug)
        ...
        for local_page in range(page):
            ...
            loc.text = domain + "/sitemaps/sitemap_products_{}_{}.xml".format(
        ...
        lastmod = ET.SubElement(sitemap, "lastmod")
        lastmod.text = timezone.now().isoformat()
        loc.text = domain + "/sitemaps/sitemap_others_{}.xml".format(country_code)
