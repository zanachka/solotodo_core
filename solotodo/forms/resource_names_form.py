...
class ResourceNamesForm(forms.Form):
    ...
    names = forms.MultipleChoiceField(choices=choices, required=False)

    def get_objects(self, request, include_permissions=False):
        resource_names = self.cleaned_data["names"]
        if not resource_names:
            resource_names = [choice[0] for choice in ResourceNamesForm.choices]
        ...
        for resource_name in resource_names:
            resource_model_and_serializer = ResourceNamesForm.model_map[resource_name]
            ...
            if resource_model_and_serializer["permission"]:
                model_objects = get_objects_for_user(
                    user, resource_model_and_serializer["permission"], klass=model
                )
            else:
                model_objects = model.objects.all()

            resource_entries = serializer(
                model_objects, many=True, context={"request": request}
            )

            if include_permissions and resource_model_and_serializer["has_permissions"]:
                perms_checker.prefetch_perms(model_objects)
                for idx, entry in enumerate(resource_entries.data):
                    entry["permissions"] = perms_checker.get_perms(model_objects[idx])
