...
class BaseProductsBrowseForm(forms.Form):
    ...
    def __init__(self, user, *args, **kwargs):
        self.user = user
        super(BaseProductsBrowseForm, self).__init__(*args, **kwargs)
    ...
    def _get_base_search(self, category=None):
    ...
    def _get_specs_form(self, request, category=None):
        if category:
            ...
            specs_form = specs_form_class(request.query_params)
        else:
    ...
    def get_category_products(self, request, category=None):
        ...
        specs_form = self._get_specs_form(request, category=category)
        assert specs_form.is_valid()

        entities_filter = self._get_entities_filter()
        search = self._get_base_search(category=category)

        all_specs_filter = specs_form.get_filter()
        search, sort_params = self._customize_search(
            search, specs_form, entities_filter
        )
    ...
    def get_category_entities(self, category, request):
        ...
        search = search.post_filter(specs_form.get_filter())
