...
def cell_plan_main_points(elastic_search_result):
    ...
    result = [
        cell_plan_navigation_quota_pretty_display(
    ...
    if elastic_search_result["minutes_free"]:
        if elastic_search_result["minutes_free"] == -1:
            ...
            result.append(
                "{} minutos todo destino".format(elastic_search_result["minutes_free"])
            )
    if elastic_search_result["sms_free"]:
        if elastic_search_result["sms_free"] == -1:
            ...
            result.append(
                "{} SMS todo destino".format(elastic_search_result["sms_free"])
            )
...
def additional_es_fields(elastic_search_result, model_name):
    ...
    if m == "Cell":
        ...
        if elastic_search_result["weight"]:
            result["pretty_weight"] = "{} g.".format(elastic_search_result["weight"])
        else:
        ...
        result["default_bucket"] = result["base_model_internal_storage_ram_key"]
        ...
        if elastic_search_result["back_camera_value"]:
            back_camera = "{} MP".format(elastic_search_result["back_camera_value"])
        ...
        if elastic_search_result["front_camera_value"]:
            front_camera = "{} MP".format(elastic_search_result["front_camera_value"])
        ...
        if elastic_search_result["operating_system_line_is_discontinued"]:
            warnings.append(
                "El sistema operativo con el que este celular "
        ...
        if elastic_search_result["line_brand_unicode"] == "Huawei":
            warnings.append(
                "Los smartphones Huawei no tienen disponibles las "
                "aplicaciones de Google (Youtube, GMail, etc) "
                "ni acceso a la Play Store."
            )
        if (
            elastic_search_result["operating_system_line_unicode"]
            == "Google Android Go"
        ):
            warnings.append(
        ...
        if (
            elastic_search_result["category_unicode"] == "Smartphone"
            and elastic_search_result["ram_value"] < 4096
        ):
            warnings.append(
                "Este equipo solo tiene {} de RAM. SoloTodo "
                "recomienda equipos con por lo menos 4 GB de RAM "
                "para un celular actual.".format(elastic_search_result["ram_unicode"])
            )
        if (
            elastic_search_result["category_unicode"] == "Smartphone"
            and elastic_search_result["internal_storage_value"] < 65536
        ):
            warnings.append(
                "Este equipo solo tiene {} de memoria. SoloTodo "
