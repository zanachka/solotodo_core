...
class Budget(models.Model):
    ...
    user = models.ForeignKey(
        get_user_model(), on_delete=models.CASCADE, related_name="budgets"
    )
    ...
    def expanded_products_pool(self):
        ...
        for entry in self.entries.filter(selected_product__isnull=False).select_related(
            "selected_product"
        ):
    ...
    def export(self, export_format):
        stores = [
            entry.selected_store
            for entry in self.entries.filter(
                selected_store__isnull=False
            ).select_related("selected_store")
        ]

        entities = (
            Entity.objects.filter(
                product__in=self.expanded_products_pool(), store__in=stores
            )
            .get_available()
            .order_by("active_registry__offer_price")
            .select_related(
                "store__country__number_format",
        ...
        if export_format == "xls":
            ...
            return self._export_as_bbcode(product_store_to_cheapest_entity_dict)
        elif export_format == "img":
            return self._export_as_img(product_store_to_cheapest_entity_dict)
        else:
    ...
    def select_cheapest_stores(self, stores):
        entities = (
            Entity.objects.filter(
                product__in=self.products_pool.all(), store__in=stores
            )
            .get_available()
            .order_by("active_registry__offer_price")
        ...
        for budget_entry in self.entries.filter(selected_product__isnull=False):
    ...
    def remove_product(self, product):
        ...
        self.entries.filter(selected_product=product).update(
            selected_product=None, selected_store=None
        )
    ...
    def compatibility_issues(self):
        ...
        * Unnecessary parts, for example adding a cpu cooler to a budget that
        ...
        entries = self.entries.filter(selected_product__isnull=False).select_related(
        ...
        products_dict = {p.id: p.category.storescraper_name for p in selected_products}
        ...
        for component_list, component_name in single_components:
            if len(component_list) > 1:
                errors.append("Tu cotización tiene más de un {}".format(component_name))
...
tarjeta mide {} mm. y el gabinete aguanta hasta {} mm.)
                                    """.format(
                                video_card.specs.length, max_case_video_card_length
                            )
                        )
...
verificar su compatibilidad con el gabinete.
        ...
        if not video_cards:
            ...
            if processor_has_integrated_graphics is False:
                errors.append(
                    """
Tu cotización no tiene tarjeta de video y el procesador no tiene gráficos integrados
                """
                )
...
La {} no permite SLI / Crossfire
                """.format(
                        vc1.specs.gpu_unicode
                    )
                )
            else:
                gpu_brand = vc1.specs.gpu_brand_unicode
                if gpu_brand == "NVIDIA" and mb and not mb.specs.allows_sli:
                    errors.append(
                        """
La placa madre no permite SLI
                    """
                    )
                elif gpu_brand == "AMD" and mb and not mb.specs.allows_cf:
                    errors.append(
                        """
...
El sistema solo puede verificar la compatibilidad de 0, 1 o 2 tarjetas de video
        ...
        if processor and mb:
            ...
            for x in getattr(
                mb.specs, "chipset_supported_processor_cores_by_default", []
            ):
            ...
            for x in getattr(
                mb.specs, "chipset_supported_processor_cores_with_bios_update", []
            ):
                update_cores.append(x.id)

            if (
                processor.specs.socket_socket_id
...
""".format(
                        ...
                        mb.specs.chipset_northbridge_family_socket_socket_unicode,
                    )
                )
...
de la placa madre.
""".format(
                        mb.name, processor.name
                    )
                )
        if processor and processor.specs.cooler_name == "No posee" and not cooler:
            errors.append(
                """
...
Tu cotización incluye {} sticks de RAM, pero la placa madre solo aguanta {}
                """.format(
                        total_dimm_count, mb.specs.memory_slots_quantity
                    )
                )

            for ram in rams:
                if (
                    ram.specs.bus_bus_bus_format_id
...
La RAM {} es de formato {} pero la placa madre usa RAMs {}
                    """.format(
                            ram.name,
                            ram.specs.bus_bus_bus_format_unicode,
                            mb.specs.memory_slots_mtype_itype_format_unicode,
                        )
                    )

                if (
                    ram.specs.bus_bus_bus_type_id
            ...
            La RAM {} es de tipo {} pero la placa madre usa RAMs {}
                                """.format(
                            ram.name,
                            ram.specs.bus_bus_bus_type_unicode,
            ...
            if (
                mb.specs.chipset_northbridge_family_socket_socket_id == 106523
                and mb.specs.memory_slots_mtype_itype_type_id == 130770
            ):
                warnings.append(
                    """
...
Tu cotización include {} discos duros pero el gabinete solo aguanta {}
                """.format(
                        len(hdds), case.specs.internal_3_1_2_bays
                    )
                )
...
El cooler no es compatible con el socket de la placa madre.
            ...
            if processor and processor.specs.socket_socket_id not in cooler_socket_ids:
                errors.append(
                    """
...
necesario.
        ...
        if mb and ssds:
            ...
            for mb_has_port in mb.specs["storage_ports"]:
                mb_available_ports.extend([mb_has_port] * mb_has_port.quantity)
...
""".format(
                    ...
                    if (
                        ssd["ssd_type_connector_id"] != 1559371
                        and port["port_connector_id"] != 1559371
                    ):
                        ...
                        if (
                            ssd["ssd_type_connector_id"] != port["port_connector_id"]
                            and ssd["ssd_type_connector_id"]
                            not in port_additional_compat_ids
                        ):
                            local_errors.append(
                                """
El puerto {} no es físicamente compatible con el ssd {}""".format(
                                    port["port_unicode"], ssd["unicode"]
                                )
                            )
...
""".format(
                            ...
                            local_errors.append(
                                """
El puerto {} es de tipo M.2 pero el SSD {} no es M.2""".format(
                                    port["port_unicode"], ssd["unicode"]
                                )
                            )
                    ...
                    for bus in getattr(port, "port_buses", []):
                        ...
                        for mb_bus in bus.to_dict().get(
                            "bus_with_version_backwards_compatibility", []
                        ):
                            mb_bus_version_compatibilities.append(mb_bus["id"])

                        if (
                            ssd["ssd_type_bus_bus_with_version_id"]
                            not in mb_bus_version_compatibilities
                        ):
                            bw_compatiblity_ids = []
                            for x in ssd.get(
                                "ssd_type_bus_bus_with_version_backwards_compatibility",
                                [],
                            ):
...
""".format(
                                        port["port_unicode"], ssd["ssd_type_unicode"]
                                    )
                                )
                        ...
                        if (
                            bus["bus_with_version_bus_id"] == 1559281
                            and ssd["ssd_type_bus_bus_with_version_bus_id"] == 1559281
                        ):
                            if bus["lanes"] == 1:
                                bus_warnings.append(
                                    """
...
el SSD puede que no funcione a plena capacidad""".format(
                                        port["port_unicode"],
                                        bus["lanes"],
                                        ssd["unicode"],
                                        ssd["ssd_type_bus_unicode"],
                                    )
                                )

                        buses_warnings_and_errors.append(
                            {"bus": bus, "warnings": bus_warnings, "errors": bus_errors}
                        )
                ...
                if warning_and_error_per_port:
                    ...
                    errors.append(
                        "No hay puertos disponible para el SSD "
                        "{}".format(ssd["unicode"])
                    )
...
El monitor {} no tiene entradas de video digital (e.g. DVI, HDMI o
 ...
 y no son compatibles con el puerto VGA.""".format(
                        monitor.name
                    )
                )
        ...
        if mb and fans:
            mb_rgb_header_ids = [
                x["header_id"] for x in mb.specs.to_dict().get("rgb_headers", [])
            ]
            for fan in fans:
                for illumination_control in fan.specs.to_dict().get(
                    ...
                    if (
                        "motherboard_header_id" in illumination_control
                        and illumination_control["motherboard_header_id"]
                        not in mb_rgb_header_ids
                    ):
                        warnings.append(
                            "El ventilador {} usa un conector {} que no es compatible con la placa madre".format(
                                fan["name"], illumination_control["unicode"]
                            )
                        )
    ...
    def _export_as_xls(self, product_store_to_cheapest_entity_dict):
        ...
        standard_format = workbook.add_format({"font_name": "Verdana", "font_size": 10})

        header_format = workbook.add_format(
            {"font_name": "Verdana", "font_size": 10, "bold": True}
        )
        ...
        for idx, header in enumerate(
            ["Componente", "Producto", "Tienda", "Precio oferta", "Precio normal"]
        ):
        ...
        for entry in self.entries.select_related(
            "selected_product__instance_model", "selected_store", "category"
        ):
            worksheet.write(row, COMPONENT_COLUMN, str(entry.category), standard_format)
            ...
            if entry.selected_store:
                if matching_entity:
                    ...
                    worksheet.write(
                        row, STORE_COLUMN, str(entry.selected_store), standard_format
                    )
            else:
                worksheet.write(row, STORE_COLUMN, "N/A", standard_format)

            if matching_entity:
                money_format = currency_to_money_format.get(matching_entity.currency)
                if not money_format:
                    ...
                    currency_to_money_format[matching_entity.currency] = money_format
                ...
                normal_price_sum += matching_entity.active_registry.normal_price
            else:
                worksheet.write(row, OFFER_PRICE_COLUMN, "N/A", standard_format)
                worksheet.write(row, NORMAL_PRICE_COLUMN, "N/A", standard_format)
        ...
        if currency_to_money_format:
            ...
            worksheet.write(row, OFFER_PRICE_COLUMN, offer_price_sum, bold_money_format)
            worksheet.write(
                row, NORMAL_PRICE_COLUMN, normal_price_sum, bold_money_format
            )
        else:
        ...
        path = storage.save(
            "budgets/exports/{}.xlsx".format(slugify(self.name)), file_for_upload
        )
    ...
    def _export_as_bbcode(self, product_store_to_cheapest_entity_dict):
        context = self.__bbcode_and_img_context(product_store_to_cheapest_entity_dict)
    ...
    def _export_as_img(self, product_store_to_cheapest_entity_dict):
        ...
        data = {"key": settings.GRABZIT_KEY, "format": "jpg", "html": rendered_html}
        http_response = requests.post(
            "https://api.grabz.it/services/convert", data=data
        )
        ...
        for entry in self.entries.select_related(
            ...
            if matching_entity:
                ...
                formatted_normal_price = currency.format_value(
                    matching_entity.active_registry.normal_price, number_format
                )
